...
export const getAvion = async (req, res) => {
    ...
    if(!getAvion) return res.status(404).json({message : 'Avion no encontrado'});
    res.json(getAvion);
    console.log(req.body)
}

export const putAvion = async (req,res) => {
    const putAvion = await AvionSchema.findByIdAndUpdate(req.params.id, req.body, {new : true});
    if (!putAvion) return res.status(404).json({message : 'Avion no encontrado'});
    res.json(putAvion);
...
export const deleteAvion = async (req, res) => {
    ...
    if (!deleteAvion) return res.status(404).json({message : 'Avion no encontrado'});
